...
do
  ...
  results_file_name=`echo "${wav_file_path/.wav/"_BirdNET_Analyzer_results_overlap_1s_ds15.csv"}"`  
  temp_species_list_file_name=`echo "${wav_file_path/_filtered.wav/"_BirdNET_Analyzer_species_list.csv"}"`
  temp2_species_list_file_name=`echo "${temp_species_list_file_name/Filtered\//""}"`
  species_list_file_name=`echo "../../${temp2_species_list_file_name/..\/..\/..\//""}"`
  analysis_command=`echo "python3 ../../BirdNET-Analyzer/analyze.py --i $wav_file_path --o $results_file_name --week $week_number --slist $species_list_file_name --overlap 1.0 --sensitivity 1.5"`
  eval "$analysis_command"
  echo "Results obtained successfully for recording $wav_file_path"
done < <(tail -n +2 ../../../CSVs/BirdNET_Analyzer/ds15/Filtered/Overlap_1s/recordings_yet_to_analyze.csv)
