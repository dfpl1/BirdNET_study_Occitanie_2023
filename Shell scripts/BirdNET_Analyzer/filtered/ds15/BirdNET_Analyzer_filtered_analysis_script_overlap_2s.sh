...
do
  ...
  results_file_name=`echo "${wav_file_path/.wav/"_BirdNET_Analyzer_results_overlap_2s_ds15.csv"}"`  
  ...
  analysis_command=`echo "python3 ../../BirdNET-Analyzer/analyze.py --i $wav_file_path --o $results_file_name --week $week_number --slist $species_list_file_name --overlap 2.0 --sensitivity 1.5"`
  eval "$analysis_command"
  echo "Results obtained successfully for recording $wav_file_path"
done < <(tail -n +2 ../../../CSVs/BirdNET_Analyzer/ds15/Filtered/Overlap_2s/recordings_yet_to_analyze.csv)
